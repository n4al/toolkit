...
# Not author-supplied code. NOT yet executed/verified in R by this Toolkit.
...
mil <- fit_component(military ~ reserves + gni + tech, Gamma(link="log"))
...
sem <- piecewiseSEM::psem(
  ...
  glm(military ~ reserves + gni + tech, family=Gamma(link="log"), data=cc),
...
cat("IMPORTANT: this script is an attempt, not a verified reproduction.\n")
